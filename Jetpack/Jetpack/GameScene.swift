    ...
    override func didMove(to view: SKView) {
        ...
        func createEnemySprite() {
            ...
            addChild(enemy)
                ...
                let dx = self.player.position.x - self.enemy.position.x
                let dy = self.player.position.y - self.enemy.position.y
                let angle = atan2(dy, dx)
                
                // Set the enemy's rotation to face the player
                self.enemy.zRotation = angle - CGFloat.pi / 2 // Subtract pi/2 to adjust for image orientation
                
                // Move the enemy towards the player's position
                let moveToPlayer = SKAction.move(to: self.player.position, duration: 2.0)
                self.enemy.run(moveToPlayer)
            
            
            // Repeat the followPlayer action forever
            let followForever = SKAction.repeatForever(moveToPlayer)
            enemy.run(followForever)
        }
    }
